...
INT8U ch_hist_init();
INT8U ch_hist_putchar(INT8U char1, INT8U *err);
INT8U ch_hist_getchar(INT8U ix, INT8U *err);
INT8U ch_hist_flush(INT8U *err);
...
void print_status_task(void* pdata)
{
  ...
  { 
    ...
    printf("****************************************************************\n");
    printf(" System time ms = %lu\n",OSTimeGet());
    OSSchedLock();
    OSQQuery(msgqueue,&result);
...
void getsem_task2(void* pdata)
{
    ...
    while (1)
  {
	led=led<<1;
	if(led==0) led=1;
	IOWR_ALTERA_AVALON_PWM_DUTY(MY_PWM_BASE,led);
    OSSemPend(shared_resource_sem, 0, &return_code);
...
void send_task(INT32U value)
{
  ...
  OS_Q_DATA queue_data;  
    ...
    {                                       
      return_code = OSQPostOpt(msgqueue, msg, OS_POST_OPT_BROADCAST);
      //alt_ucosii_check_return_code(return_code);
...
void receive_task1(void* pdata)
{
  ...
  while (1)
  {
    ...
    printf(" receive_task1. BUTTON = %lu\n",msg);
    //alt_ucosii_check_return_code(return_code);
    //if(return_code==OS_NO_ERR)
    	number_of_messages_received_task1++;
...
void receive_task2(void* pdata)
{
  ...
  {
    msg = (INT32U *)OSQPend(msgqueue, 0, &return_code);
    printf(" receive_task2. BUTTON = %lu\n",msg);
   // alt_ucosii_check_return_code(return_code);
    //if(return_code==OS_NO_ERR)
    	number_of_messages_received_task2++;
    OSTimeDlyHMSM(0, 0, 1, 0);
...
int initCreateTasks(void)
{
  ...
  alt_ucosii_check_return_code(return_code);
 // return_code = OSTaskCreate(send_task,  NULL, (void *)&send_task_stk[TASK_STACKSIZE-1], SEND_TASK_PRIORITY);
 // alt_ucosii_check_return_code(return_code);
